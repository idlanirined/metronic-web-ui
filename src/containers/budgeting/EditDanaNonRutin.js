...
export default function EditDananNonRutin() {
    ...
    const getData = () => {
        let dataHeadOffice = location.state.dataHeadOffice
        let idx = dataHeadOffice.dana_non_rutin.findIndex((val) => val.id === location.state.selected[7])
        setDataTable(dataHeadOffice.dana_non_rutin[idx].dataNonRutin)
        ...
        let idReg = newListRegion.findIndex((val) => val.name === location.state.selected[3])
        setRegion(newListRegion[idReg])
